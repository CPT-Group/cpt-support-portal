...
- **Teams webhook notification includes submitter name** – The support submission Teams webhook message now includes the submitter's first name, last name, case name, and request type(s) (previously only case name and request types). Updated `notifySupportSubmissionTeams` interface and the support-request route to pass `firstName` and `lastName` from the submission body. Webhook URL consolidated to a single Teams channel; set `SUPPORT_SUBMISSION_WEBHOOK_URL` in env to the active URL.
- **Jira NOVA-733 updated with support request testing checklist** – Added a comment to [NOVA-733](https://cptgroup.atlassian.net/browse/NOVA-733) with: (1) instructions for Thomas to run a full end-to-end submission test for each of the 17 support request types (submit via portal, confirm route/API, screenshot the resulting record in Salesforce); (2) a table of all request types with Required fields, Optional fields, and a blank Passed Testing column to fill in. Credentials from `.env.jira.temp`; script payload at `scripts/jira-nova-733-comment.json`; comment posted via Jira Cloud REST API v3 (`curl.exe`).

### Fixed

- **Optional fields (CPT ID) no longer block submission (NOVA-733)** – When a user triggered validation errors (e.g. submitting with a missing required field), the `_general` summary error ("Please fill in all required fields: ...") persisted even after the user corrected the field, making it appear that CPT ID or other optional fields were blocking submission. Two fixes: (1) Extracted a pure `getFieldValidationError()` helper so `validateStep` no longer calls `validateField` (which had `setErrors` side effects); `validateStep` now makes a single clean `setErrors(newErrors)` call with no intermediate state updates from the validation loop. (2) `updateFormData` now clears the `_general` summary error whenever any field is edited, so stale validation messages don't persist after the user fixes issues. Reported by Thomas Williams during QA.
...
- **Project__c now links to the selected case** – Portal submissions now set `Project__c` to the selected case's Project record ID (from the dropdown) instead of a hardcoded "Support" bucket project. This means `Case_Name__c` (formula from Project__c) auto-populates with the correct case name (e.g., "Brandi-Vanmeter v. MP2 Enterprises, LLC"). Removed the `[Case: ...]` prepend workaround from `Website_Detail_Summary__c`. Falls back to `SUPPORT_CHANNEL_DEFAULT_PROJECT_ID` env var if no case is selected.
- **File uploads to Salesforce (ContentVersion)** – Supporting documentation files attached in the portal are now uploaded to Salesforce as ContentVersion records and linked to the Support_Channel__c record via ContentDocumentLink. Files appear in the "Files" related list on the record in Salesforce. The form submission uses FormData (multipart) when files are present, falling back to JSON when no files are attached. Each file is uploaded individually after the record is created; upload failures are logged but never fail the submission. The portal enforces a 5MB per-file limit.
- **Request_Type__c multipicklist support** – Switched from single-select `Type__c` to multi-select `Request_Type__c` (created by Salesforce admin). The portal supports selecting multiple request types; the API now resolves ALL selected labels to picklist API values and joins them with semicolons (Salesforce multipicklist format). Updated `PORTAL_TO_SF_FIELD`, resolver function, retry logic, verify script, and docs. Also confirmed `Additional_Description__c` is now live and createable.
- **Separate case email/phone from member email/phone** – The class member's personal email and phone were incorrectly mapped to `Case_Email__c` / `Case_Phone__c` (case-level contact fields). Now: (1) case list query pulls `Email__c` and `Phone__c` from Project__c so case contact info is available; (2) case email/phone auto-populates `Case_Email__c` / `Case_Phone__c` from the selected Project record; (3) the form's member email/phone maps to dedicated `Email__c` / `Phone__c` fields on Support_Channel__c (created by Salesforce admin). Updated CaseOption type, jsonGenerator, PORTAL_TO_SF_FIELD, verify script, and salesforce.md.
...
- **Type__c restricted picklist error (500 on submit)** – Salesforce `describe` returns all picklist values including inactive ones; the Type__c resolution was matching inactive values and sending them, causing `INVALID_OR_NULL_FOR_RESTRICTED_PICKLIST` on create. Now filters to `active !== false` only. Added logging of resolved Type__c value and active picklist values for debugging. If the create still fails with a restricted-picklist error, the route retries without Type__c (field is nillable) so submissions succeed even when the picklist is out of sync with the portal's 17 request types. Also: JWT private key PEM normalization for Netlify (single-line PEM reshaped), and 500 responses always return a JSON body with `Content-Type: application/json` so the client toast shows the real error.
- **OAuth access token auto-refresh** – `sfFetchWithStoredToken()` in `salesforceOAuth.ts` now automatically refreshes the access token on a 401 response using the stored `refresh_token`, saves the new tokens to `.sf_tokens.json`, and retries the original request once. Previously, expired access tokens (~2 hour Salesforce session timeout) required manually re-running `/oauth/start`; now the refresh happens transparently. Extracted `refreshAccessToken()` (shared by both file-based and env-based flows), `saveTokensToFile()`, `buildSfUrl()`, and `parseSfError()` helpers. In-memory cache (`cachedTokensFromEnv`) is cleared on 401 so stale tokens don't persist.
- **Loading overlay not showing during case list fetch** – The loading overlay was missing when navigating to the case selection step (step 1) because the overlay was triggered via `useEffect` (which runs after the browser paint), so the user always saw one frame of the empty dropdown first. Replaced the effect-based approach with **direct JSX rendering**: the overlay (`<div className="app-loading-overlay">` with ProgressSpinner) is rendered inline when `activeStep === 1 && cases.length === 0 && !casesError`, so it appears in the same paint as the step content with zero gap. The submit overlay still uses `useEffect` + `useLoadingOverlay()` context (fine for user-initiated actions).
...
- **Home page FAQ button and theme secondary colors** – The FAQ button on the home page now uses the themed secondary style by adding `p-button-secondary` with `p-button-outlined`, so it picks up `--button-secondary-*` from the active theme (e.g. neon purple outline in dark-synth). Every theme now defines its own secondary button variables: **light** (slate blue), **dark** (navy/slate), **cpt-legacy-light** (CPT blue-gray), **cpt-legacy-dark** (CPT slate), **ms-access-2010** (Office blue-gray), **dark-synth** (existing neon purple with glow). Secondary buttons and outlined secondary buttons (e.g. FAQ, feedback dialogs) now align with each theme.
- **Salesforce tokens: no repeated OAuth, 24/7 with refresh only** – Token logic was updated so you never have to hit OAuth/start repeatedly. (1) **When `SF_REFRESH_TOKEN` is set** (e.g. Netlify), the app uses only that env var to get access tokens; it never reads the token file, so production works 24/7 without the public URL or file. (2) **When using the token file locally**, the app now refreshes the access token **before** it expires (if older than 90 minutes) instead of waiting for a 401, so rerunning the app or submitting the next day works without re-doing OAuth. (3) Docs: added a note that the Salesforce Connected App should set **Refresh Token Validity** to "Refresh token is valid until revoked" so the refresh token does not expire. OAuth/start is only needed once (on localhost) to obtain the refresh_token; then set `SF_REFRESH_TOKEN` in production and the portal is usable by the public without any further auth.
- **POST /api/support-request on Netlify (500 fix)** – The route no longer calls `readStoredTokens()` for the Salesforce create step; it uses `sfFetchWithStoredToken` for the create as well, so tokens come from `.sf_tokens.json` locally or from `SF_REFRESH_TOKEN` on serverless (Netlify). Previously the create step read the token file directly, which doesn’t exist on Netlify and caused an uncaught exception and empty 500. All Salesforce calls (describe, userinfo, create) now use the same token path (file or env refresh). Errors are caught and returned as JSON so the client always gets a `{ success: false, message }` body instead of an empty 500.
- **PrimeReact ripple on buttons** – Ripple effect is now enabled globally via `PrimeReactProvider` (`ripple: true`). Buttons already include PrimeReact’s `Ripple` component; with the config on, the click ripple animation is active. Button root styles in `primereact-overrides.scss` were updated with `position: relative` and `overflow: hidden` so the ripple is contained and visible.
- **Outlined secondary button text/icon on dark themes** – Outlined secondary buttons (e.g. FAQ on home) were using `--button-secondary-text` for label and icons; in dark-synth that is dark (#07071b), so the text was invisible on the dark background. Overrides now use `--button-secondary-bg` for label and icons on `.p-button-secondary.p-button-outlined` (and hover), so the accent color is used and the text is readable in dark-synth and other dark themes.
- **Button icon centering** – Icons inside buttons (e.g. thumbs up/down in FAQ feedback) were slightly off-center due to icon font baseline/line-height. Overrides in `primereact-overrides.scss` for `.p-button .p-button-icon` and `[data-pc-section='icon']`: `display: inline-flex`, `align-items: center`, `justify-content: center`, `line-height: 1` so icons are visually centered in both icon-only and icon+label buttons.
- **Address / AutoComplete dropdown button uses theme colors** – The address field’s dropdown trigger (and other input-group icon-only buttons) now use the theme’s secondary button variables (`--button-secondary-bg`, `--button-secondary-text`, etc.) so they match the active theme. In dark-synth the button uses neon purple with dark text; in other themes it uses their secondary (surface-based or theme-specific). Overrides in `primereact-overrides.scss` for `.p-autocomplete-dropdown.p-button` and `.p-inputgroup .p-button.p-button-icon-only`.
- **Submission Data (Prototype) on success page behind feature flag** – The “Submission Data (Prototype)” card at the end of the support-request success/confirmation page is now gated by `NEXT_PUBLIC_SHOW_SUBMISSION_DATA_PROTOTYPE`. Set it to `true` in `.env.local` (or deploy env) to show the JSON block; leave unset or false to hide it. Default is off.
- **Loading overlay – single overlay for all loading states, optional message** – Case list step now uses the main loading overlay instead of an inline overlay: `showLoading('Loading case list')` while cases load, same full-screen themed overlay as route and submit. One effect in `SupportRequestStepper` drives overlay from both `casesLoading` and `isSubmitting` with the appropriate message (submit takes precedence). API: `showLoading()` uses default "Loading, please wait..."; `showLoading('Custom message')` for custom copy (e.g. "Loading case list", "Submitting your request..."). Removed inline ProgressSpinner and mask from the case selection step.
