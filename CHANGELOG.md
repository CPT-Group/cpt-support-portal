...
- **Loading overlay positioning** – Full-screen loading overlays (route transitions and support-request submit) now cover the entire viewport. Shared CSS in `globals.css` for `.global-loading-overlay` and `.loading-overlay` uses explicit viewport dimensions (`100vw`/`100vh`), `top/left/right/bottom: 0`, and `!important` so overlays are not shaped or shifted by parent layout.
